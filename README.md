# KnowHub（知枢）· RAG 知识库 Agent

基于大模型的检索增强生成（RAG）知识库问答平台：上传文档自动建立知识库，通过**向量 + 关键词混合检索 + LLM 重排**精准召回，以**带引用溯源（文档/页码）的流式问答**回答用户问题，内置多租户、RBAC 权限、操作审计、异步索引等能力。
...
- 📊 系统能力：操作审计、站内通知、数据仪表盘、检索调试台、模型自检、限流、敏感信息检测与脱敏
